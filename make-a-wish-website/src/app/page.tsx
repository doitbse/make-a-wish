...
export default function Home() {
  ...
  const handleExport = () => {
    const headers = ["Plan", "Users", "MRR", "Status"];
    const csvContent = [
      headers.join(","),
      ...filteredRows.map((r) =>
        [r.plan, `"${r.users}"`, `"${r.mrr}"`, r.status].join(",")
      ),
    ].join("\
");
    ...
    link.setAttribute("download", `acme_analytics_export_${new Date().toISOString().split('T')[0]}.csv`);
